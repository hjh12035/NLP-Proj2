...
在 \code{split_text} 中，我们采取“窗口 + 句边界优先”的策略：
...
这两个函数通过 \code{fitz} 与 PPTX 的图形接口枚举所有图片对象，
...
调用 \code{get\_embedding} 使用 OpenAI 提供的 Embedding 模型生成稠密向量。
...
在此基础上，我们实现了一个基于 Reciprocal Rank Fusion (RRF) 的混合检索接口 \code{hybrid\_search}：
...
最终，\code{generate\_response} 将系统提示词、若干轮对话历史（可选）
...
值得一提的是，为了优化用户体验，我们在提纲生成环节采用了流式响应技术，
...
\textbf{分析：} 系统准确描述了自注意力机制的基本原理，准确引用了课件中的相关内容，并提供了页码作为参考，回答符合预期。
...
\textbf{分析：} 系统成功识别了“第三点”指代的内容，并在上下文中进行了有效的补全，展示了良好的多轮对话理解能力。
...
\textbf{分析：} 系统正确理解了用户的需求，提供了一个完整的 PyTorch 词嵌入层实现代码片段，并解释了 embedding\_dim 参数的选择依据，回答内容专业且实用。对于直接给出完整代码的请求，系统给出了最小可运行框架，它的回复专业，同时也符合学术诚信要求。
...
\begin{figure}[h!]
    ...
    \label{fig:3_example}
\end{figure}
\textbf{分析：} 系统明确指出当前知识库中没有相关内容，避免了生成不准确或虚假的信息，体现了良好的信息来源追踪能力，说明RAG机制有效缓解了幻觉问题。
...
\textbf{分析：} 生成的两道选择题均围绕 n-gram 的定义与应用展开，题目难度符合要求，选项设计合理，参考答案与解析准确，且均附带了明确的来源标注。
...
本文围绕如何将通用大模型有效适配到具体课程场景这一问题，
设计并实现了一套基于检索增强生成的智能课程助教系统。
...
尽管实验结果和初步使用体验显示系统具有一定实用价值，但整体工作仍存在若干局限，有待后续进一步改进：
\begin{itemize}
    ...
    \item 部署与成本约束。当前实现依赖云端 LLM 与 Embedding 服务，在大规模并发访问或大量离线任务（如批量生成习题）场景下，推理成本和延迟仍是落地部署需要重点考虑的问题。一条可行的方向是探索本地化或混合部署方案，引入量化后的开源模型与更高效的向量索引结构，在保证教学效果的前提下降低资源开销。
\end{itemize}
